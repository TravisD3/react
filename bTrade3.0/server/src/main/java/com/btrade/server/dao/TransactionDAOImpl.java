...
public class TransactionDAOImpl implements TransactionDAO, InitializingBean {
	...
	// private static final String CUSTOMER_INERT_SQL = "INSERT INTO CUSTOMER(ID,
	// FIRSTNAME, LASTNAME)";
	// private static final String CUSTOMER_UPDATE_SQL = "UPDATE CUSTOMER SET ID
	// WHERE ID = ?";
	// private static final String CUSTOMER_DELETE_SQL = "DELETE FROM CUSTOMER WHERE
	// ID = ?";
	...
	{
		System.out.println("*******************生成TransactionDAOImpl的Bean");
	}
...
//	}
	...
	public void setSessionFactory(SessionFactory sessionFactory) {
		...
		System.out.println("***************给TransactionDaoImpl注入SessionFactory*********");
	}
	...
	public void saveUser2(int userId, String firstName, String lastName) {
		...
		try {
			...
			System.out
					.println("IN saveUser HibernateOptimisticLockingFailureException exception NEW today= +++++++++=");
		}
	}
	...
	public List<Transaction> listTransaction() {
		return this.hibernateTemplate.find("from Transaction");
	}

	public List<Transaction> getTransactionByAccountNum(String accountNum) {
		if (accountNum.equals("all")) {
			return this.hibernateTemplate.find("from Transaction");
		}
		return this.hibernateTemplate.find("from Transaction where ACCT_NUM = ?", new Object[] { accountNum });
	}

	public List<Transaction> getTransactionByAccountNum(String accountNum, String status) {
		if (accountNum.equals("all")) {
			return this.hibernateTemplate.find("from Transaction where STATUS = ?", new Object[] { status });
		}
		return this.hibernateTemplate.find("from Transaction where ACCT_NUM = ? And STATUS = ?",
				new Object[] { accountNum, status });
	}

	public List<Transaction> getTransactionByAccountNum(String accountNum, String status, Date beginDate,
			Date endDate) {
		if (accountNum.equals("all")) {
			return this.hibernateTemplate.find("from Transaction where STATUS = ? And TRADE_DATE between ? and ?",
					new Object[] { status, beginDate, endDate });
		}
		return this.hibernateTemplate.find(
				"from Transaction where ACCT_NUM = ? And STATUS = ? And TRADE_DATE between ? and ?",
				new Object[] { accountNum, status, beginDate, endDate });
	}
	...
	public List<Transaction> listAllTransactions() {
		System.out.println("listAllTransactions...");
		List<Transaction> list = this.hibernateTemplate.find("select distinct t.acctNum from Transaction t");
		return list;
	}
	...
	public void setTxManager(HibernateTransactionManager txManager) 
	{
		...
		System.out.println("***************给TransactionDaoImpl注入TransactionManager*********");
	}
}
